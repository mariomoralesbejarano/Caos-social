...
export default function CaosLobbyScreen() {
  const router = useRouter();
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const isWeb = Platform.OS === "web";
  const { hydrated, setSession } = useRoom();
  ...
  return (
    ...
    >
      ...
      <Pressable
        ...
        style={[styles.hubBack, { borderColor: colors.border }]}
      >
        <Text style={{ fontSize: 16 }}>🏠</Text>
        <Text style={[styles.hubBackText, { color: colors.mutedForeground }]}>
          Menú Principal
        </Text>
      </Pressable>
...
const styles = StyleSheet.create({
  ...
  hubBackText: { fontFamily: "Inter_500Medium", fontSize: 13 },
  hero: { gap: 8 },
