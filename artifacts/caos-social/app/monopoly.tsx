    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={[styles.container, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 28 }]}>
        <View style={styles.header}><View><Text style={styles.kicker}>MONOPOLY SOCIAL · {room.code}</Text><Text style={[styles.title, { color: colors.foreground }]}>La ciudad es tuya</Text></View><Pressable onPress={leave} style={styles.out}><Text style={styles.outText}>SALIR</Text></Pressable></View>
         ...
         <View style={styles.turnCard}><Text style={[styles.turn, { color: isMyTurn ? "#55d6ff" : colors.mutedForeground }]}>{isMyTurn ? "TU TURNO" : `Turno de ${current ?? "otro jugador"}`}</Text><Text style={styles.move}>{gs.lastMove}</Text>{gs.partyMode && gs.partyEvent ? <View style={styles.partyEvent}><Text style={styles.partyKicker}>MODO FIESTA / BEBER</Text><Text style={styles.partyText}>{gs.partyEvent}</Text></View> : null}</View>
  ...
  return (
    <View style={styles.board}>
      ...
      {state.spaces.map((space) => {
        ...
        return (
          <View key={space.id} style={[styles.space, (space.id % 10 === 0) && styles.cornerSpace, { left: `${point.x}%`, top: `${point.y}%` }]}>
            {space.color ? <View style={[styles.spaceStrip, { backgroundColor: PROPERTY_COLORS[space.color] }]} /> : null}
